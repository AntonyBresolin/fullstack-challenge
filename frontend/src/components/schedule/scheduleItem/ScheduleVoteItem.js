...
const ScheduleVoteItem = ({schedule}) => {
  return (
    <div className='flex justify-between items-center border mt-8 pb-4 flex-col bg-gray-50 w-72 h-64 rounded-2xl'>
      <h3 className='text-xl font-medium text-center bg-emerald-800 py-2 w-full text-white rounded-t-xl'>{schedule.title}</h3>
      ...
      <button className='bg-emerald-800 hover:bg-emerald-900 text-white rounded-lg px-4 py-2 mt-4 duration-150 ease-in-out'>Votar</button>
    </div>
  );
};
